...
| `MAX_DOC_PAGES` | сколько страниц разбирать | 400 |
| `CACHE_SIZE` | сколько разобранных документов помнить | 64 |
| `SERVICE_API_KEY` | ключ доступа к сервису, пустой означает открытый доступ | пусто |
| `RATE_PER_HOUR` | сколько разборов в час с одного адреса | 30 |
...
27 тестов: разбор денег и дат в русских формулировках, извлечение текста, распознавание скана без текстового слоя, гашение инструкций внутри документа, кеш по хешу файла, лимиты размера и числа страниц, проверка ключа доступа, ограничение частоты и то, что тело ответа провайдера не уезжает клиенту.

Сервис проверялся и вживую, не только тестами: `/health`, отказ на битом файле, отказ на скане без текстового слоя, отказ при отсутствии ключа модели.
